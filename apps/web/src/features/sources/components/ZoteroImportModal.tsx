  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t("zoteroImport.parseError"));
      } finally {
        setIsParsing(false);
      }
    },
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-2xl bg-card text-card-foreground border border-border rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden font-sans">
        ...
        <div className="overflow-y-auto p-6 space-y-6 bg-card/50">
          <p className="text-muted-foreground text-sm">{t("zoteroImport.instructions")}</p>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <p className="text-sm text-muted-foreground">{t("zoteroImport.clickToUpload")}</p>
              {fileContent && (
                <p className="text-xs text-primary">{t("zoteroImport.fileLoaded")}</p>
              )}
            </div>
          </div>
          ...
          {isParsing && (
            <div className="flex items-center justify-center gap-2 py-4">
              ...
              <p className="text-sm text-muted-foreground">{t("zoteroImport.parsing")}</p>
            </div>
          )}
            ...
            <div className="space-y-4">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-sm truncate">
                          {paper.title || t("zoteroImport.untitled")}
                        </p>
              ...
              {newPapers.length === 0 && (
                <div className="bg-secondary/20 rounded-lg p-4 text-center">
                  <p className="text-sm text-muted-foreground">
                    {t("zoteroImport.allAlreadyInNotebook")}
                  </p>
                </div>
              )}
              ...
              {newPapers.length > 0 && (
                ...
                >
                  ...
                  {t("zoteroImport.importPapers", { count: newPapers.length })}
                </button>
              )}
            </div>
          )}
