...
export const reportGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.studio._helpers.updateReportStatus, {
        reportId,
        status: 'generating',
        metadata: {
          phase: 'initializing',
      ...
      await ctx.runMutation(internal.studio._helpers.initReportMapPhase, {
        reportId,
      ...
      await ctx.runMutation(internal.studio._helpers.markReportFailed, {
        reportId,
...
2. "summary": The complete structured summary as described above`;
      ...
      await ctx.runMutation(internal.studio._helpers.storeReportMapResult, {
        reportId,
        chunkIndex,
...
export const finalizeReportPhase = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      // Save results
      await ctx.runMutation(internal.studio._helpers.saveReportResults, {
        reportId,
        content,
        metadata: {
          title,
      ...
      await ctx.runMutation(internal.studio._helpers.clearReportMapData, { reportId });
