...
import { packChunks, validateChunks } from '../lib/agents/SlideDeckGraph';
...
} from '../lib/agents/slides/prompts';
import {
  invokeWithTimeout,
  invokeWithRetry,
  sanitizeUserInput,
...
export const slideDeckGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.initSlideDeckMapPhase, {
        slideDeckId,
      ...
      for (let i = 0; i < packedChunks.length; i++) {
        await ctx.scheduler.runAfter(0, internal.jobs.SlideDeckGenerationJob.processSlideDeckMapChunk, {
          slideDeckId,
          userId,
          notebookId,
          chunkIndex: i,
...
export const processSlideDeckMapChunk = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const slideDeck = await ctx.runQuery(internal.slides.getInternal, { id: slideDeckId });
      if (!slideDeck) {
      ...
      await ctx.runMutation(internal.jobs.helpers.storeSlideDeckMapResult, {
        slideDeckId,
        chunkIndex,
      ...
      const updatedSlideDeck = await ctx.runQuery(internal.slides.getInternal, { id: slideDeckId });
      ...
      if (completedMaps >= totalMaps) {
        ...
        await ctx.scheduler.runAfter(0, internal.jobs.SlideDeckGenerationJob.finalizeSlideDeckPhase, {
          slideDeckId,
          userId,
          notebookId,
          slideCount,
...
export const finalizeSlideDeckPhase = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.clearSlideDeckMapData, { slideDeckId });
