...
import { packChunks, validateChunks } from '../lib/agents/WrittenQuestionsGraph';
...
} from '../lib/agents/written_questions/prompts';
import {
  invokeWithTimeout,
  invokeWithRetry,
  sanitizeUserInput,
...
export const writtenQuestionsGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.initWrittenQuestionsMapPhase, {
        writtenQuestionId,
      ...
      for (let i = 0; i < packedChunks.length; i++) {
        await ctx.scheduler.runAfter(0, internal.jobs.WrittenQuestionsGenerationJob.processWrittenQuestionsMapChunk, {
          writtenQuestionId,
          userId,
          notebookId,
          chunkIndex: i,
...
export const processWrittenQuestionsMapChunk = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const writtenQuestion = await ctx.runQuery(internal.writtenQuestions.getInternal, { id: writtenQuestionId });
      if (!writtenQuestion) {
      ...
      await ctx.runMutation(internal.jobs.helpers.storeWrittenQuestionsMapResult, {
        writtenQuestionId,
        chunkIndex,
      ...
      const updatedWrittenQuestion = await ctx.runQuery(internal.writtenQuestions.getInternal, { id: writtenQuestionId });
      ...
      if (completedMaps >= totalMaps) {
        ...
        await ctx.scheduler.runAfter(0, internal.jobs.WrittenQuestionsGenerationJob.finalizeWrittenQuestionsPhase, {
          writtenQuestionId,
          userId,
          notebookId,
          questionCount,
...
Select exactly ${questionCount} questions.`;
      ...
      await ctx.runMutation(internal.jobs.helpers.clearWrittenQuestionsMapData, { writtenQuestionId });
