"use node";
/**
 * SpreadsheetGenerationJob - Multi-Phase Architecture
 *
 * Breaks spreadsheet generation into separate scheduled actions to avoid
 * Cloudflare 524 timeouts. Each phase runs in its own action with
 * its own timeout window.
 *
 * Phases:
 * 1. spreadsheetGeneration (entry) - Load docs, pack chunks, schedule map tasks
 * 2. processSpreadsheetMapChunk - Extract text/summaries from one chunk (parallel)
 * 3. finalizeSpreadsheetPhase - Collapse outputs, generate CSV, save output
 */

import { internalAction } from '../_generated/server';
import { v } from 'convex/values';
import { internal } from '../_generated/api';
import { packChunks, validateChunks } from '../lib/agents/SpreadsheetGraph';
...
} from '../lib/agents/spreadsheet/prompts';
import {
  invokeWithTimeout,
  invokeWithRetry,
  sanitizeUserInput,
...
export const spreadsheetGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.initSpreadsheetMapPhase, {
        spreadsheetId,
      ...
      for (let i = 0; i < packedChunks.length; i++) {
        await ctx.scheduler.runAfter(0, internal.jobs.SpreadsheetGenerationJob.processSpreadsheetMapChunk, {
          spreadsheetId,
          userId,
          notebookId,
          chunkIndex: i,
...
export const processSpreadsheetMapChunk = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const spreadsheet = await ctx.runQuery(internal.spreadsheets.getInternal, { id: spreadsheetId });
      if (!spreadsheet) {
      ...
      await ctx.runMutation(internal.jobs.helpers.storeSpreadsheetMapResult, {
        spreadsheetId,
        chunkIndex,
      ...
      const updatedSpreadsheet = await ctx.runQuery(internal.spreadsheets.getInternal, { id: spreadsheetId });
      ...
      if (completedMaps >= totalMaps) {
        ...
        await ctx.scheduler.runAfter(0, internal.jobs.SpreadsheetGenerationJob.finalizeSpreadsheetPhase, {
          spreadsheetId,
          userId,
          notebookId,
          spreadsheetType,
...
export const finalizeSpreadsheetPhase = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.clearSpreadsheetMapData, { spreadsheetId });
