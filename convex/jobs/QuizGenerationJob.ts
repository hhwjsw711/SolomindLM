"use node";
/**
 * QuizGenerationJob - Multi-Phase Architecture
 *
 * Breaks quiz generation into separate scheduled actions to avoid
 * Cloudflare 524 timeouts. Each phase runs in its own action with
 * its own timeout window.
 *
 * Phases:
 * 1. quizGeneration (entry) - Load docs, pack chunks, schedule map tasks
 * 2. processQuizMapChunk - Generate candidates for one chunk (parallel, own timeout)
 * 3. finalizeQuizPhase - Select best candidates, expand to full questions, save
 */

import { internalAction } from '../_generated/server';
import { v } from 'convex/values';
import { internal } from '../_generated/api';
import { packChunks, validateChunks } from '../lib/agents/QuizGraph';
...
function createMapLLM(): ChatTogetherAI {
  return new ChatTogetherAI({
    ...
    maxTokens: parseInt(env.QUIZ_MAX_TOKENS || '8000', 10),
  });
}
...
export const quizGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.initQuizMapPhase, {
        quizId,
      ...
      for (let i = 0; i < packedChunks.length; i++) {
        await ctx.scheduler.runAfter(0, internal.jobs.QuizGenerationJob.processQuizMapChunk, {
          quizId,
          userId,
          notebookId,
          chunkIndex: i,
...
export const processQuizMapChunk = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const quiz = await ctx.runQuery(internal.quizzes.getInternal, { id: quizId });
      if (!quiz) {
      ...
      await ctx.runMutation(internal.jobs.helpers.storeQuizMapResult, {
        quizId,
        chunkIndex,
      ...
      const updatedQuiz = await ctx.runQuery(internal.quizzes.getInternal, { id: quizId });
      ...
      if (completedMaps >= totalMaps) {
        ...
        await ctx.scheduler.runAfter(0, internal.jobs.QuizGenerationJob.finalizeQuizPhase, {
          quizId,
          userId,
          notebookId,
          questionCount,
...
export const finalizeQuizPhase = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const selectedCandidates = (selectionResponse as QuizCandidateResponse).questions;
      ...
      await ctx.runMutation(internal.jobs.helpers.clearQuizMapData, { quizId });
