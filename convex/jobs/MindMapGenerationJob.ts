"use node";
/**
 * MindMapGenerationJob - Multi-Phase Architecture
 *
 * Breaks mind map generation into separate scheduled actions to avoid
 * Cloudflare 524 timeouts. Each phase runs in its own action with
 * its own timeout window.
 *
 * Phases:
 * 1. mindmapGeneration (entry) - Load docs, pack chunks, schedule map tasks
 * 2. processMindMapMapChunk - Extract concepts from one chunk (parallel)
 * 3. finalizeMindMapPhase - Build hierarchical structure, save output
 */

import { internalAction } from '../_generated/server';
import { v } from 'convex/values';
import { internal } from '../_generated/api';
import { packChunks, validateChunks } from '../lib/agents/MindMapGraph';
...
import { z } from 'zod';
import { MAP_PROMPT, REDUCE_PROMPT, MAP_SYSTEM_PROMPT, REDUCE_SYSTEM_PROMPT } from '../lib/agents/mindmap/prompts';
import type { ConceptExtraction, MindMapNode, FinalMindMap } from '../lib/agents/mindmap/state';
import {
  invokeWithTimeout,
  invokeWithRetry,
...
export const mindmapGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.initMindMapMapPhase, {
        mindmapId,
      ...
      for (let i = 0; i < packedChunks.length; i++) {
        await ctx.scheduler.runAfter(0, internal.jobs.MindMapGenerationJob.processMindMapMapChunk, {
          mindmapId,
          userId,
          notebookId,
          chunkIndex: i,
...
export const processMindMapMapChunk = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const mindmap = await ctx.runQuery(internal.mindmaps.getInternal, { id: mindmapId });
      if (!mindmap) {
      ...
      await ctx.runMutation(internal.jobs.helpers.storeMindMapMapResult, {
        mindmapId,
        chunkIndex,
      ...
      const updatedMindmap = await ctx.runQuery(internal.mindmaps.getInternal, { id: mindmapId });
      ...
      if (completedMaps >= totalMaps) {
        ...
        await ctx.scheduler.runAfter(0, internal.jobs.MindMapGenerationJob.finalizeMindMapPhase, {
          mindmapId,
          userId,
          notebookId,
        });
...
export const finalizeMindMapPhase = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.jobs.helpers.clearMindMapMapData, { mindmapId });
