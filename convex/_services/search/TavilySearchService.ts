...
export class TavilySearchService {
  ...
  async discoverSources(options: DiscoveryOptions): Promise<DiscoveredSource[]> {
    ...
    const legacyLog = createServiceLogger("tavily", "discoverSourcesLegacy");
    legacyLog.operationStart({ queryLen: query.length, topic });
    ...
    try {
      ...
      legacyLog.operationComplete({ count: sources.length, scoreThreshold });
