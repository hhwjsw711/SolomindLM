...
/**
 * Options for academic paper discovery
 */
...
/**
 * OpenAlexSearchService handles academic paper discovery using OpenAlex API
 * Optimized for finding scholarly works with rich metadata
 *
 * @deprecated Use discoverAcademicPapers action directly for caching support
 */
export class OpenAlexSearchService {
  private baseUrl: string;

  constructor() {
    this.baseUrl = env.OPENALEX_BASE_URL || "https://api.openalex.org";
  }
   ...
   * @returns Array of academic papers sorted by relevance score
   * @deprecated Use discoverAcademicPapers action for caching support
   */
  async discoverAcademicPapers(options: AcademicDiscoveryOptions): Promise<AcademicPaper[]> {
    const {
      query,
      maxResults = 20,
      publicationYear,
    ...
    const legacyLog = createServiceLogger("openalex", "discoverAcademicPapersLegacy");
    legacyLog.operationStart({ queryLen: query.length });

    try {
      // Build filter parameters
      const filters: string[] = [];

      if (publicationYear?.from) {
        filters.push(`publication_year:>${publicationYear.from - 1}`);
      }
      if (publicationYear?.to) {
        filters.push(`publication_year:<${publicationYear.to + 1}`);
      }
      if (minCitations) {
        filters.push(`cited_by_count:>${minCitations - 1}`);
      }
      ...
      // Build URL
      const filterParam = filters.length > 0 ? `&filter=${filters.join(',')}` : '';
      const url = `${this.baseUrl}/works?search=${encodeURIComponent(query)}${filterParam}${sortParam}&per_page=${maxResults}`;

      legacyLog.apiCall("openalex", "/works", { path: "works" });
      ...
      const papers: AcademicPaper[] = (data.results || []).map((work: any) => {
        ...
        // Extract open access info
        const openAccess = work.open_access?.is_oa || false;
        const hasFullTextPaper = !!work.best_oa_location?.pdf_url ||
                               !!work.best_oa_location?.landing_page_url ||
                               openAccess;
        ...
        // Calculate score (combination of citation count and recency)
        const citationScore = Math.min(work.cited_by_count || 0, 1000) / 1000;
        const recencyScore = publicationYear ? Math.min(publicationYear, 2024) / 2024 : 0.5;
        const score = (citationScore * 0.7) + (recencyScore * 0.3);
      ...
      legacyLog.operationComplete({ count: papers.length });

      return papers;

    } catch (error) {
      legacyLog.operationError(error);

      if (error instanceof Error) {
        throw new Error(`Academic paper discovery failed: ${error.message}`);
      }

      throw new Error("Academic paper discovery failed: Unknown error");
    }
  }
}
