 ...
 */
function transformTavilyResult(result: any, sourceType: 'web' | 'news' | 'finance'): UnifiedDiscoveryResult {
  return {
...
export const discover = action({
  args: {
    ...
    academicFilters: v.optional(v.object({
  ...
  handler: async (ctx, args) => {
    ...
    for (const topic of tavilyTopics) {
      ...
      const promise = (ctx.runAction as any)(internal._services.search.TavilySearchService.discoverSourcesInternal, {
        query,
    ...
    if (searchAcademic) {
      ...
      const promise = (ctx.runAction as any)(internal._services.search.OpenAlexSearchService.discoverAcademicPapersInternal, {
        query,
        maxResults: maxResults,
    ...
    logger.info("Source performance", {
      breakdown: searchResults.map(
        (r) => `${r.sourceType}:${r.duration}ms:${r.results.length}`
      ),
    });
    ...
    return {
      ...
      sourceTypeCounts: searchResults.reduce((acc, { sourceType, results }) => {
...
export const discoverSources = action({
  ...
  handler: async (ctx, args): Promise<{ sources: Array<{ title: string; url: string; snippet: string; score: number }> }> => {
    const result = await (ctx.runAction as any)(internal._services.search.TavilySearchService.discoverSourcesInternal, {
      query: args.query,
